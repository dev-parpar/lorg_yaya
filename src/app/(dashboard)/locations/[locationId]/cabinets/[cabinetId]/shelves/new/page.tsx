...
export default async function NewShelfPage({ params }: Props) {
  ...
  return (
    <div>
      <nav className="flex items-center gap-1 text-sm text-muted-foreground mb-6 flex-wrap">
        ...
        <ChevronRight className="h-3.5 w-3.5" />
        <span className="text-foreground">New Shelf</span>
      </nav>
      <PageHeader title="Add Shelf" description={`Adding to: ${cabinet.name}`} />
      <ShelfForm cabinetId={cabinetId} locationId={locationId} />
    </div>
  );
}
