...
export default async function NewCabinetPage({ params }: Props) {
  ...
  return (
    <div>
      <nav className="flex items-center gap-1 text-sm text-muted-foreground mb-6">
        ...
        <ChevronRight className="h-3.5 w-3.5" />
        <span className="text-foreground">New Cabinet</span>
      </nav>
      <PageHeader title="Add Cabinet" description={`Adding to: ${location.name}`} />
      <CabinetForm locationId={locationId} />
    </div>
  );
}
